...
def main(data_path):
    ...
    enron_submits = src.utils.minmax_time(enron)
    seattle = pd.read_csv(seattle_path).to_numpy()
    seattle_submits = src.utils.minmax_time(seattle)
    print('enron:', enron_submits)
    print('seattle:', seattle_submits)
    ...
    plt.xlabel(f"Messages per receiver", fontweight="bold", fontsize=24)
    plt.ylabel("Probability Density",
               fontweight="bold", fontsize=24)
    plt.tight_layout()
    plt.legend(fontsize="xx-large")
    plt.savefig(f"receiver-pdf.pdf", format="pdf")
    plt.show()


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="Receiver figure", description="This code produces a figure of messages by receiver.")
    parser.add_argument(
